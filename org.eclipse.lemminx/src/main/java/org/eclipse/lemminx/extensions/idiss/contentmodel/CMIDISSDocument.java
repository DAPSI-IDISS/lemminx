...
/**
 * IDISS document implementation.
 *
 */
public class CMIDISSDocument implements CMDocument, XSElementDeclHelper {

	private static final Logger LOGGER = Logger.getLogger(CMIDISSDocument.class.getName());

	private final XSModel model;

	private final Map<XSElementDeclaration, CMIDISSElementDeclaration> elementMappings;
	...
	public CMIDISSDocument(XSModel model, XSLoaderImpl xsLoaderImpl) {
		this.model = model;
	...
	private static FilesChangedTracker createFilesChangedTracker(XSModel model) {
		...
		return IDISSUtils.createFilesChangedTracker(grammars);
	}
	...
	void collectElement(XSElementDeclaration elementDeclaration, Collection<CMElementDeclaration> elements) {
		if (elementDeclaration.getAbstract()) {
			...
			CMElementDeclaration cmElement = getIDISSElement(elementDeclaration);
	...
	CMElementDeclaration getIDISSElement(XSElementDeclaration elementDeclaration) {
		CMIDISSElementDeclaration element = elementMappings.get(elementDeclaration);
		if (element == null) {
			element = new CMIDISSElementDeclaration(this, elementDeclaration);
	...
	public LocationLink findTypeLocation(DOMNode originNode) {
		...
		// Try to retrieve IDISS element declaration from the given element.
		CMIDISSElementDeclaration elementDeclaration = (CMIDISSElementDeclaration) findCMElement(originElement,
		...
		if (URIUtils.isFileResource(documentURI)) {
			...
			if (originAttribute != null) {
				...
				CMIDISSAttributeDeclaration attributeDeclaration = (CMIDISSAttributeDeclaration) elementDeclaration
	...
	/**
	 * Returns the owner schema grammar of the given IDISS element declaration and
	 * null otherwise.
	 *
	 * @param elementDeclaration the IDISS element declaration
	 * @return the owner schema grammar of the given IDISS element declaration and
	 *         null otherwise.
	 */
	SchemaGrammar getOwnerSchemaGrammar(XSElementDeclaration elementDeclaration) {
		...
		// 1) when IDISS element declaration has namespace, it is bound with the grammar.
		...
		// 2) IDISS element declaration has none namespace. Here we loop for each grammar
		// to discover if the element type belong to this grammar.
		XSNamespaceItemList namespaces = model.getNamespaceItems();

		// 2.1) IDISS global element, uses the SchemaGrammar#getElementDeclaration method
		...
		// 2.2) IDISS local element, get the parent xs:complexType of the IDISS element and
	...
	private static ElementImpl findLocalMappedXercesElement(XSElementDeclaration elementDeclaration,
		...
		try {
			...
			// IDISSHandler.
			// Xerces stores the location in the IDISSHandler instance in 2 arrays:
			...
			// Get the IDISSHandler instance from the XMLSchemaLoader instance
			XSDHandler handler = ReflectionUtils.getFieldValue(schemaLoader, "fSchemaHandler");

			// Get the XSParticleDecl array from the IDISSHandler instance
			...
			if (i >= 0) {
				// Get the Xerces Element array from the IDISSHandler instance
	...
	private static LocationLink findXSAttribute(DOMAttr originAttribute, NodeList children, boolean inAnyLevel) {
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (n.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (IDISSUtils.isXSAttribute(elt)) {

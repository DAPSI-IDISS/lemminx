...
/**
 * IDISS element declaration implementation.
 *
 */
public class CMIDISSElementDeclaration implements CMElementDeclaration {

	private static final short PC_UNKWOWN = -1;

	private final CMIDISSDocument document;
	...
	public CMIDISSElementDeclaration(CMIDISSDocument document, XSElementDeclaration elementDeclaration) {
		...
		if (list != null) {
			for (int i = 0; i < list.getLength(); i++) {
				...
				if (object.getType() == XSConstants.ATTRIBUTE_USE) {
					...
					attributes.add(new CMIDISSAttributeDeclaration(attributeUse));
				}
			}
		}
	}
	...
	public String getDocumentation(ISharedSettingsRequest request) {
		...
		this.documentation = new IDISSDocumentation(annotations, docStrategy, !markdownSupported)
	...
	public Collection<String> getEnumerationValues() {
		...
		if (typeDefinition != null) {
			...
			return CMIDISSDocument.getEnumerationValues(simpleDefinition);
		}
		return Collections.emptyList();
	}
	...
	private Map<String, String> createTextsDocumentation(ISharedSettingsRequest request) {
		...
		getEnumerationValues().forEach(value -> {
			...
			if (annotations != null) {
				documentation = new IDISSDocumentation(annotations, value, docStrategy, !markdownSupported)
						.getFormattedDocumentation(markdownSupported);
			}
	...
	private XSObjectList getTextAnnotations(String textContent) {
		...
		if (typeDefinition != null) {
			...
			return CMIDISSDocument.getEnumerationAnnotations(simpleDefinition, textContent);
		}
		return null;
	}
	...
	public String getDocumentURI() {
		...
		return CMIDISSDocument.getSchemaURI(schemaGrammar);
	}
}

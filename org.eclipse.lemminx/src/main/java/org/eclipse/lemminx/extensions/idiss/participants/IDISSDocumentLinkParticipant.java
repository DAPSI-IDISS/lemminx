...
/**
 * 
 * Implements document links in .idiss files for
 * <ul>
 ...
 * 
 */
public class IDISSDocumentLinkParticipant implements IDocumentLinkParticipant {

	private static final Logger LOGGER = Logger.getLogger(IDISSDocumentLinkParticipant.class.getName());

	@Override
	public void findDocumentLinks(DOMDocument document, List<DocumentLink> links) {
		DOMElement root = document.getDocumentElement();
		if (!IDISSUtils.isXSSchema(root)) {
			return;
		}
		...
		for (DOMNode child : children) {
			if (child.isElement() && Objects.equals(child.getPrefix(), xmlSchemaPrefix)) {
				DOMElement idissElement = (DOMElement) child;
				if (IDISSUtils.isXSInclude(idissElement) || IDISSUtils.isXSImport(idissElement)) {
					DOMAttr schemaLocationAttr = IDISSUtils.getSchemaLocation(idissElement);

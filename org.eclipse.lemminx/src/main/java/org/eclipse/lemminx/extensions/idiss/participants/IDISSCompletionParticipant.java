...
import org.eclipse.lemminx.extensions.idiss.DataType;
 ...
 *
 */
public class IDISSCompletionParticipant extends CompletionParticipantAdapter {
	...
	public void onAttributeValue(String valuePrefix, ICompletionRequest request, ICompletionResponse response,
		...
		BindingType bindingType = IDISSUtils.getBindingType(originAttr);
		if (bindingType != BindingType.NONE) {
			// Completion on
			// - @type (ex : xs:element/@type)
			// - @base (ex : xs:extension/@base)
			// bound to complextTypes/@name
			IDISSUtils.searchXSTargetAttributes(originAttr, bindingType, false, true,

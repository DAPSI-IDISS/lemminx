...
*******************************************************************************/
package org.eclipse.lemminx.extensions.idiss.utils;
...
public class IDISSUtils {
	...
	/**
	 * Collect IDISS target attributes declared in the XML Schema according the given
	 ...
	 * @param collector              collector to collect IDISS target attributes.
	 */
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (originAttributes != null) {
					for (int j = 0; j < originAttributes.getLength(); j++) {
						...
						BindingType originBnding = IDISSUtils.getBindingType(originAttr);

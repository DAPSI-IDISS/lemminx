 ...
 *
 */
public enum IDISSErrorCode implements IXMLErrorCode {
	...
	private IDISSErrorCode() {
		this(null);
	}

	private IDISSErrorCode(String code) {
		this.code = code;
	}
	...
	private final static Map<String, IDISSErrorCode> codes;

	static {
		codes = new HashMap<>();
		for (IDISSErrorCode errorCode : values()) {
			codes.put(errorCode.getCode(), errorCode);
		}
	}

	public static IDISSErrorCode get(String name) {
	 ...
	 */
	public static Range toLSPRange(XMLLocator location, IDISSErrorCode code, Object[] arguments, DOMDocument document) {
		...
		case schema_reference_4: {
			return XMLPositionUtility.selectAttributeValueAt(IDISSUtils.SCHEMA_LOCATION_ATTR, offset, document);
		}
		case EmptyTargetNamespace :
			return XMLPositionUtility.selectAttributeValueAt(IDISSUtils.TARGET_NAMESPACE_ATTR, offset, document);
		case src_import_3_1: {
			...
			if (elementHighlighted.getNodeName().equals(IDISSUtils.XS_SCHEMA_TAG)) { // `xs:schema` line is highlighted
				return XMLPositionUtility.selectChildNodeAttributeValueFromGivenNameAt(IDISSUtils.XS_IMPORT_TAG, IDISSUtils.NAMESPACE_ATTR, offset, document);
			} else { // `xs:import` line is highlighted
				return XMLPositionUtility.selectAttributeValueAt(IDISSUtils.NAMESPACE_ATTR, offset, document);
			}
		}
		case src_import_3_2:
			return XMLPositionUtility.selectChildNodeAttributeValueFromGivenNameAt(IDISSUtils.XS_IMPORT_TAG, IDISSUtils.SCHEMA_LOCATION_ATTR, offset, document);
		}

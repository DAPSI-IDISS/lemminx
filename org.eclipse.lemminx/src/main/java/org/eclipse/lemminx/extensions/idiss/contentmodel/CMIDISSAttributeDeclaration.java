...
/**
 * IDISS attribute declaration implementation.
 *
 */
public class CMIDISSAttributeDeclaration implements CMAttributeDeclaration {
	...
	public CMIDISSAttributeDeclaration(XSAttributeUse attributeUse) {
	...
	public String getDefaultValue() {
		...
		if (xsValue == null) {
			if (CMIDISSDocument.isBooleanType(getAttrDeclaration().getTypeDefinition())) {
				return "false";
			}
		}
	...
	public String getAttributeNameDocumentation(ISharedSettingsRequest request) {
		...
		this.attrDocumentation = new IDISSDocumentation(annotations, docStrategy, !markdownSupported)
	...
	private XSObjectList getAttributeValueAnnotations(String value) {
		...
		XSObjectList annotation = CMIDISSDocument.getEnumerationAnnotations(simpleTypeDefinition, value);
		if (annotation != null && annotation.getLength() > 0) {
			return annotation;
		}
	...
	public Collection<String> getEnumerationValues() {
		...
		if (attributeDeclaration != null) {
			XSSimpleTypeDefinition typeDefinition = attributeDeclaration.getTypeDefinition();
			return CMIDISSDocument.getEnumerationValues(typeDefinition);
		}

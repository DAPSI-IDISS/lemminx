...
public class DataType {
	...
	public static String getDocumentation(DOMAttr attr) {
		...
		if (IDISSUtils.isXSComplexType(element)) {
			doc.append(lineSeparator);
			doc.append(" - Type: `Complex Type` ");
		} else if (IDISSUtils.isXSSimpleType(element)) {
			doc.append(lineSeparator);
	...
	private static Map<String, DataType> loadDataTypes() {
		try {
			...
			saxParser.parse(new InputSource(DataType.class.getResourceAsStream("/schemas/idiss/datatypes.xml")), handler);

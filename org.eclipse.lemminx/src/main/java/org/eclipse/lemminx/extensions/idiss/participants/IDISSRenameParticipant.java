 ...
 * 
 */
public class IDISSRenameParticipant implements IRenameParticipant {
	...
	private List<TextEdit> getRenameTextEdits(IRenameRequest request) {
		...
		if (IDISSUtils.isXSComplexType(ownerElement) || IDISSUtils.isXSSimpleType(ownerElement)) {
	...
	private List<Location> getReferenceLocations(DOMNode node) {
		...
		IDISSUtils.searchXSOriginAttributes(node,

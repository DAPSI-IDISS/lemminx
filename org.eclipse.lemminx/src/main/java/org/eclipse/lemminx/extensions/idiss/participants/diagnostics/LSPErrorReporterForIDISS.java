...
import org.eclipse.lemminx.extensions.idiss.participants.IDISSErrorCode;
 ...
 *
 */
public class LSPErrorReporterForIDISS extends AbstractLSPErrorReporter {

	private static final String IDISS_DIAGNOSTIC_SOURCE = "idiss";

	public LSPErrorReporterForIDISS(DOMDocument xmlDocument, List<Diagnostic> diagnostics, boolean hasRelatedInfo) {
		super(IDISS_DIAGNOSTIC_SOURCE, xmlDocument, diagnostics, hasRelatedInfo);
	}
	...
	protected Range toLSPRange(XMLLocator location, String key, Object[] arguments, String message,
		...
		// try adjust positions for IDISS error
		IDISSErrorCode idissCode = IDISSErrorCode.get(key);
		if (idissCode != null) {
			Range range = IDISSErrorCode.toLSPRange(location, idissCode, arguments, document);
			if (range != null) {
				return range;
			}
		}
